(define (stat-files p size)
  (let loop
    (lambda (tuple max-size file)
            (tuple max-length name)
            (tuple max-elements path)
            (tuple max-literal-len literal)
            total
            meta-size
            n-files
            total-elements
            ...
            (let ((str (read-line p)))
              (if (eof-object? str)
                ...
                (let ((l (string-split str #\space))
                      ...
                      (lit (car (sort literals (lambda (x y) (> (string-length x) (string-length y))))))
                      (lit-len (string-length lit)))
                  (loop (tuple (max max-size file-size)
                               (if (> file-size max-size) file-path file))
                        (tuple (max max-length len)
                               (if (> len max-length) file-path name))
                        (tuple (max max-elements n-elements)
                               (if (> n-elements max-elements) file-path path))
                        (tuple (max max-literal-len lit-len)
                               (if (> lit-len max-literal-len) lit literal))
                        (+ total file-size)
                        (+ 1 len meta-size)
                        (+ 1 n-files)
                        (+ total-elements n-elements)
                        (fold + total-elements-len (map string-length literals)))))))))
    (loop (tuple 0 "") (tuple 0 "") (tuple 0 "") (tuple 0 "") 0 0 0 0 0 0)))
